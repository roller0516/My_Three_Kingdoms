...
//		private string 			sourcePathRoot;
        ...
        private UnityEditor.iOS.Xcode.PBXProject xcodeProject;
		...
		private PBXList _targetBuildPhases;
...
//					Debug.LogWarning ("skipping " + buildConfigName + " config " + (string)b.data["name"]);
		...
		public bool AddHeaderSearchPaths( string path, bool recursive )
		{
			...
			path = path.Replace('\\', '/');
            xcodeProject.UpdateBuildProperty(targetGUID, "HEADER_SEARCH_PATHS", new string[] { path}, new string[] { });
...
//							Debug.Log( "Adding Copy Files Build Phase" );
        ...
        private void AddBuildFile(PBXFileReference fileReference, bool weak)
        {
            //string sourcePath = Path.Combine(projectRootPath, fileReference.path);
            string fileGuid = xcodeProject.AddFile(fileReference.path, fileReference.path);
            xcodeProject.AddFileToBuildWithFlags(targetGUID, fileGuid, string.Join(" ", fileReference.compilerFlags));
        }

        private void AddBuildFramework(PBXFileReference fileReference, bool weak)
        {
            if (fileReference.name.EndsWith(".a", System.StringComparison.InvariantCulture) || !fileReference.path.StartsWith("System/Library/Frameworks", System.StringComparison.InvariantCulture))
            {
                AddBuildFile(fileReference, weak);
            }
            else
            {
                xcodeProject.AddFrameworkToProject(targetGUID, fileReference.name, weak);
            }
        }
...
//			Debug.Log( "Adding linker flags..." );
		...
		*/
		...
		public PBXNativeTarget GetUnityNativeTarget()
		{
			string name = "Unity-iPhone";
        ...
        private UnityEditor.iOS.Xcode.PBXProject GetXcodeProject(string path)
        {

            xcodeProjectPath = UnityEditor.iOS.Xcode.PBXProject.GetPBXProjectPath(path);
        ...
        private string GetUnityTargetGUID()
        {
            string unityFrameworkTargetGUID = xcodeProject.GetUnityFrameworkTargetGuid();

            /*var unityFrameworkTargetGUIDMethod = xcodeProject.GetType().GetMethod("GetUnityFrameworkTargetGuid");

            if (unityFrameworkTargetGUIDMethod != null)
            {
                unityFrameworkTargetGUID = (string)unityFrameworkTargetGUIDMethod.Invoke(xcodeProject, null);
            }
            else
            {
                unityFrameworkTargetGUID = xcodeProject.TargetGuidByName("Unity-iPhone");
            }*/
