...
- (void)playVideoUsingWebView:(NSString *)embedHTML {
    ...
    // Set presentation style //@@ iOS 13
    [videoPlayerVC setModalPresentationStyle: UIModalPresentationFullScreen];
...
- (void)playVideoAtURL:(NSURL *)URL fullscreen:(BOOL)fullscreen {
    ...
    [UnityGetGLViewController() presentViewController:moviePlayerVC animated:TRUE completion:^{
...
- (void)stopPlayingVideo {
	...
	// Stops webview embedded player //@ Removed here as it won't be available as its auto released in next call
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"view.frame"]) {
        CGRect updatedRect = [change[NSKeyValueChangeNewKey] CGRectValue];
        if (updatedRect.origin.y != 0) {
              NSLog(@"Video Closed");
            [[NSNotificationCenter defaultCenter] postNotificationName:AVPlayerItemFailedToPlayToEndTimeNotification object:nil userInfo:nil];
        }
     }
}
